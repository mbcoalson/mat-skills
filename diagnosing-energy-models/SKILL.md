...
**Location**: `tools/analyze_energyplus_results.py`
...
python tools/analyze_energyplus_results.py \
...
### Unmet Hours Diagnostics

**Location**: `tools/diagnose-unmet-hours.py`

Per-zone diagnostic cross-reference that mines `eplusout.sql` to correlate unmet hours with zone sizing, heating coil capacity, fan sizing, envelope exposure, ventilation fraction, and thermostat schedules.

**Usage**:
```bash
python tools/diagnose-unmet-hours.py <run-dir> [--format markdown|json|csv] [--output FILE] [--threshold N]
```

**When to Use**: First tool to run when investigating unmet hours — provides a comprehensive per-zone diagnostic snapshot.

### Multi-Run Zone Comparison

**Location**: `tools/compare-zone-diagnostics.py`

N-way comparison tool that tracks zone metrics across simulation iterations. Accepts any number of run directories with labels.

**Usage**:
```bash
python tools/compare-zone-diagnostics.py <run1> <run2> [<run3> ...] --labels "A" "B" ["C" ...] [--threshold N] [--json FILE]
```

**When to Use**: After making model changes to identify which zones improved, which worsened, and by how much.

### Heating Load Decomposition

**Location**: `tools/heating-load-decomposition.py`

Decomposes peak heating losses from the SensibleHeatGainSummary report into envelope (opaque + window), infiltration, HVAC delivery, internal gains, and interzone transfer components per zone.

**Usage**:
```bash
python tools/heating-load-decomposition.py <run-dir> [--zones "ZONE1,ZONE2"] [--format markdown|json|csv] [--output FILE] [--threshold N]
```

**When to Use**: When unmet hours diagnostics show the problem but you need to identify the dominant loss pathway (envelope vs infiltration vs ventilation).

### Capacity Gap Analysis

**Location**: `tools/capacity-gap-analysis.py`

Calculates theoretical warmup time from setback to occupied setpoint using zone volume, air density at altitude, coil capacity, and envelope UA losses. Identifies zones that cannot physically recover within the pre-start period.

**Usage**:
```bash
python tools/capacity-gap-analysis.py <run-dir> [--setback 18.3] [--setpoint 21.1] [--prestart-hours 3.0] [--altitude 1525] [--mode heating] [--format markdown|json|csv] [--output FILE] [--threshold N]
```

**When to Use**: When you suspect equipment is undersized or pre-start time is insufficient. Also reveals structurally limited zones (negative net capacity).
...
- **All tools**: Python 3.12 stdlib only (sqlite3, json, csv, os, sys, argparse, math)
- **SQL location**: Tools check both `<dir>/eplusout.sql` and `<dir>/run/eplusout.sql`
- **Output formats**: All tools support `--format markdown|json|csv`. JSON output feeds the `/xlsx` skill for Excel workbook generation.
...
**Location:** `tools/npv_analysis_email_generator.py`
...
4. **Generate formatted summary:**
   ```bash
   python tools/npv_analysis_email_generator.py "path/to/NPV_Analysis.xlsx" \
...
**Version**: 3.0 (Renamed scripts/ to tools/, added unmet hours diagnostic suite)
**Aligned with**: OpenStudio 3.10, EnergyPlus 24.2
**Primary Use Cases**: Recreation center projects, LEED v4 projects, complex HVAC troubleshooting, unmet hours root cause analysis
